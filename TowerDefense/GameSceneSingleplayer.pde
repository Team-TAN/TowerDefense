public class GameSceneSingleplayer extends Scene {
  ...
  public ArrayList<PVector> aiPos = new ArrayList<PVector>();
  ...
  public void onMousePressed() {
    if (isBuildingState) {
      ...
      if(mouseX >= 260 && mouseX <= 500 && mouseY >= 350 - tileHeight * 2 && mouseY <= 350) {
        ...
        changeScene(new GameSceneSingleplayer(difficulty));
    }
    }
  }
  ...
  private void buildingUpdate() {
    ...
    //if(computerDoneBuilding) {
      ...
      if(aiBuildTimeLeft <= 0) {
        if(aiTowers.size() > 0) {
          PVector pos = aiPos.get(aiPos.size() - 1);
          ...
          player2.selectedBuildTile = new PVector(19, 3);
  ...
  private void startMiniGame() {
    ...
    isMiniGame = true;
    miniGame = new RhythmGameSingle(player1, (int) random(0, miniGameSongs.length), maxRhythmTime, creepHealthMult);
    maxRhythmTime += 3;
    turns++;
    if(turns % 3 == 0) {
      creepHealthMult += .3f;
  ...
  private void onMiniGameEnd() {
    ...
    int numOfCreeps = (difficulty == 1 ? (int)random(totalPossibleCreeps / 2, totalPossibleCreeps) : totalPossibleCreeps);
    float startTime = 1;
    for(int i = 0; i < numOfCreeps; ++i) {
      Creep c;
      if(difficulty == 1)
        c = new Creep(random(0,1) > .5f, false, creepHealthMult * player2.creepHealthMultiplyer);
      else 
